...
{#if $modalStore[0]}
	<div class="modal px-4">
		<form action='#' on:submit|preventDefault={update}>
			<button
				class="closer"
				on:click|preventDefault={cancel}><span class="icon i-fe-close"></span></button
			>
			...
			<article>
				<div class="edit-entry">
					...
					step=1
					bind:value={newValue}
					on:keydown={(e)=>{if (e.code === 'Enter') update(e)}}>
				</div>
			</article>
			<footer>
				<div class="actions">
					<button class="btn variant-soft-error" on:click|preventDefault={cancel}>Cancel</button>
