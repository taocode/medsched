<script>
	...
	let { id, displayName, medications, timeLog } = medication
	$: {
		if (medication) ({ id, displayName, medications, timeLog } = medication)
	}
	...
	async function dispense(medicationIndex) {
		return dispenseRecipientMedication($user.uid, medication.id, medicationIndex)
	}
</script>

<div class="show-medication">
	<h2>{displayName}</h2>
	<div class="my-3">
		<div
